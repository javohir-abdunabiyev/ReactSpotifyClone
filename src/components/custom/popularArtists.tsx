...
function PopularArtists() {
    ...
    return (
        <div>
            <h1 className="text-white text-[24px] hover:underline cursor-pointer font-bold !pl-[10px] !mb-[10px]">Популярные исполнители</h1>
            <div className="flex overflow-x-scroll no-scrollbar !pr-[34px] !mb-[40px]">
                {
                    state.map((artist: any) => (
                        <div key={artist.id} className="relative flex-shrink-0 max-w-[177px] w-full h-[225px] !p-[12px] hover:bg-[#2c2b2b] rounded-[4px] cursor-pointer group">
                            <img src={artist.images[0]?.url} alt={artist.name} className="w-[153px] h-[153px] object-cover rounded-full !mb-[2px]" />
                            <Button
                                className="absolute rounded-full w-[48px] h-[48px] bg-[#1ed760] right-[20px] bottom-[70px] 
